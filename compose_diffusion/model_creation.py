...
from .respace_sampler import Sampler_SpacedDiffusion
from composable_diffusion import sampler_gd as gd_sampler
...
from composable_diffusion.unet import  SuperResUNetModel, UNetModel_full,Energy_UNetModel_full
...
def create_model(
    ...
    if raw_unet:
        if super_res:
            ...
            if energy_mode:
                model = Energy_UNetModel_full #UNetModel_full
            else:
                model = UNetModel_full
        return model(
            in_channels=3,
...
def Sampler_create_gaussian_diffusion(
    steps=1000,
    learn_sigma=False,
    sigma_small=False,
    noise_schedule="squaredcos_cap_v2",
    ...
    return Sampler_SpacedDiffusion(
        use_timesteps=space_timesteps(steps, timestep_respacing),
        betas=betas,
        model_var_type=(
            (
                gd_sampler.ModelVarType.FIXED_LARGE
                if not sigma_small
                else gd_sampler.ModelVarType.FIXED_SMALL
            )
            if not learn_sigma
            else gd_sampler.ModelVarType.LEARNED_RANGE
        ),
    )
